 ...
 *
 *	Demo Link: https://youtu.be/Naed2oYAByQ
 *
 */
#include <avr/io.h>
...
unsigned char SF_Sound;
enum SF_States { SF_Start, SF_Up, SF_Down} SF_State;
int SF_Tick(int state) {
...
int main(void) {
	...
	++i;
	tasks[i].state = SF_Start;
	tasks[i].period = 5; 
	tasks[i].elapsedTime = 0;
	tasks[i].TickFct = &SF_Tick;
	TimerSet(1);
	TimerOn();
